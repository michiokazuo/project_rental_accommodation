let tableDataNotFull, tableDataFull, btnDeleteRoom, modalDelete, modalSaveRoom, selectSort, selectCategory,
    ...
    numberPrice, dateCreateDate, dateModifyDate, numberMaxPerson, numberFloor, rate;
...
async function findHost() {
    await userFindById(idHost)
        .then(rs => {
            if (rs.status === 200) {
                host = rs.data;
                $("#header").text("Quản lý trọ của " + host.name);
            }
        })
        .catch(e => {
            console.log(e);
    ...
    btnDeleteRoom.click(async function () {
        ...
        if (check) {
            ...
            await notify_impl(emails.substring(0, emails.length - 1), "Xóa phòng/nhà trọ",
                `Trọ ${roomSave.motelRoom.title} mà bạn thuê/đang yêu cầu thuê đã bị quản lý xóa đi. 
